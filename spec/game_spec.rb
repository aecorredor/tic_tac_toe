  ...
  describe Game do
    ...
    context '#initialize' do
      it 'randomly selects a current_player' do
        ...
        # we expect frank because that's the first element of the Array players
        # that was passsed to game
    ...
    context '#get_move' do
      it 'converts the human move 1 to [0, 0]' do
        game = Game.new([bob, frank])
        expect(game.get_move('1')).to eq [0, 0]
      end

      it 'converts the human move 7 to [0, 2]' do
        game = Game.new([bob, frank])
    ...
    context '#game_over_message' do
      it "returns '{currents_player.name} won!' if board shows a winner" do
        game = Game.new([bob, frank])
        game.stub(:current_player) { bob }
